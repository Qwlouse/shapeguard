...
class DimSpec(object):
  ...
  def has_conflict(self, shape_entry, known_dims):
    ...
    If there is not enough knowledge to decide, this function will assume no
    conflict and return False.

    Args:
      shape_entry: int or None. Dim-value of the shape to be checked against.
      known_dims: Dict[str, int]. Dictionary of known named dimension sizes.

    Returns:
      True if there is a conflict (with the given knowledge), False otherwise.
    """
  ...
  def evaluate(self, known_dims):
    ...
    Raises:
      UnderspecifiedShapeError: if there is not enough information to determine
...
class Wildcard(DimSpec):
  ...
  def evaluate(self, known_dims):
    return -1
    # raise UnderspecifiedShapeError("Wildcard (*) cannot be evaluated. "
    #                                "Did you mean to use dynamic (?) instead?")
